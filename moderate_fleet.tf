...
resource "aws_cloudwatch_metric_alarm" "moderate_fleet_ecs_cluster_cpu_util_cloudwatch" {
  count               = "${var.moderate_fleet}"
...
resource "aws_cloudwatch_metric_alarm" "moderate_fleet_ecs_cluster_mem_util_cloudwatch" {
  count               = "${var.moderate_fleet}"
...
resource "aws_appautoscaling_target" "moderate_fleet_service_target" {
  count              = "${var.moderate_fleet}"
  max_capacity       = 50
  min_capacity       = "${var.cluster_size}"
  resource_id        = "spot-fleet-request/${aws_spot_fleet_request.moderate_fleet.id}"
  role_arn           = "${aws_iam_role.autoscaling_role.arn}"
  scalable_dimension = "ec2:spot-fleet-request:TargetCapacity"
  service_namespace  = "ec2"
}

resource "aws_appautoscaling_policy" "moderate_fleet_service_down_policy" {
  count              = "${var.moderate_fleet}"
  ...
  depends_on = ["aws_appautoscaling_target.moderate_fleet_service_target"]
}

resource "aws_appautoscaling_policy" "moderate_fleet_service_up_policy" {
  count              = "${var.moderate_fleet}"
...
resource "aws_cloudwatch_metric_alarm" "moderate_fleet_service_highcpu_scaleup" {
  count               = "${var.moderate_fleet}"
  ...
  alarm_actions = [
    "${aws_appautoscaling_policy.moderate_fleet_service_up_policy.arn}",
  ]

  depends_on = ["aws_appautoscaling_policy.moderate_fleet_service_up_policy"]
}

resource "aws_cloudwatch_metric_alarm" "moderate_fleet_service_highcpu_scaledown" {
  count               = "${var.moderate_fleet}"
  ...
  alarm_actions = [
    "${aws_appautoscaling_policy.moderate_fleet_service_down_policy.arn}",
  ]
